...
import type { InferenceConfig } from "./types.ts";

export class StreamSession {
  private audioBufferQueue: Uint8Array[] = [];
  private maxQueueSize = 2000; // Maximum number of audio chunks to queue
  ...
  public async streamAudio(audioData: Uint8Array): Promise<void> {
...
export class NovaSonicBidirectionalStreamClient {
  ...
  private toolsAvailable: Map<string, {toolSpecification: any, action: Function}> = new Map();


  constructor(config: BedrockRuntimeClientConfig, customInferenceConfig: InferenceConfig = DefaultInferenceConfiguration) {
   ...
   * @param toolsName - The name of the tool (case-insensitive)
   * @param toolSpecification - The tool specification object containing name, description, and inputSchema
   * @param action - The function to execute when the tool is called. Receives (sessionId, toolUseContent) as parameters
   */
  public setTool(toolsName: string, toolSpecification: any, action: Function): void {
    const normalizedKey = toolsName.toLowerCase();
    console.log(`🔧 Registering tool: "${toolsName}" → key: "${normalizedKey}" → name: "${toolSpecification.name}"`);
    
    this.toolsAvailable.set(normalizedKey, { toolSpecification, action });
    
    // Verify registration
    const registered = this.toolsAvailable.get(normalizedKey);
    if (registered) {
      console.log(`✅ Tool "${toolSpecification.name}" successfully registered with key "${normalizedKey}"`);
    } else {
      console.error(`❌ Failed to register tool "${toolSpecification.name}" with key "${normalizedKey}"`);
    }
  }
  ...
  public isToolRegistered(toolName: string): boolean {
    ...
    return this.toolsAvailable.has(normalizedKey);
  }
  ...
  public getRegisteredToolNames(): string[] {
    return Array.from(this.toolsAvailable.keys());
  }

  // Create a new streaming session
  public createStreamSession( sessionId: string = crypto.randomUUID()): StreamSession {
  ...
  private async processToolUse(
    ...
    const toolName_lc = toolName.toLowerCase();
    console.log(`🔍 Looking for tool: "${toolName}" (normalized: "${toolName_lc}")`);
    
    // Debug: Show all available tools
    console.log('🔍 Available tools:');
    this.toolsAvailable.forEach((tool, key) => {
      console.log(`  - Key: "${key}" → Name: "${tool.toolSpecification.name}"`);
    });
    
    const tool = this.toolsAvailable.get(toolName_lc);

    if (tool) {
      console.log(`✅ Found tool: "${toolName}" → executing`);
      return await tool.action(sessionId, JSON.stringify(toolUseContent));
    } else {
      console.log(`❌ Tool "${toolName}" not found in available tools`);
      console.log(`❌ Searched for key: "${toolName_lc}"`);
      console.log(`❌ Available keys: [${Array.from(this.toolsAvailable.keys()).join(', ')}]`);
  ...
  // TODO: These functions are currently unused but may be needed for future tool integration
  /*
  ...
  private async fetchWeatherData(
    ...
    try {
      ...
      console.error(
        `Error fetching weather data: ${error instanceof Error ? error.message : String(error)} `,
        error,
      );
      throw error;
    }
  ...
  */
    ...
    try {
      for await (const event of response.body) {
        ...
        if (event.chunk?.bytes) {
          ...
          console.log(`Unknown event type for session ${sessionId}: `, event);
        }
      }
      ...
      if (error.name === 'ModelTimeoutException') {
        console.error(
          `\n\n\n!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n` +
          `!!! MODEL TIMEOUT EXCEPTION DETECTED FOR SESSION ${sessionId} !!!\n` +
          `!!! Message: ${error.message} !!!\n` +
          `!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n\n\n`
        );
        this.dispatchEvent(sessionId, "modelTimedOut", {
          source: "responseStream",
          message: "Model has timed out.",
          details: error.message,
        });
      } else {
        // Dispatch a generic error for all other unrecoverable errors
        this.dispatchEvent(sessionId, "error", {
          source: "responseStream",
  ...
  public setupPromptStartEvent(sessionId: string): void {
    ...
    const session = this.activeSessions.get(sessionId);
    if (!session) return;
    // Build tools list from both dynamic tools and legacy hardcoded tools
    const toolsList = Array.from(this.toolsAvailable.entries()).map(([_key, tool]) => ({
      toolSpec: tool.toolSpecification
    }));

    console.log(`Tools available for session ${sessionId}: `, JSON.stringify(toolsList, null, 2));
  ...
  public async streamAudioChunk(
    ...
    const base64Data = toBase64(audioData);
  ...
  private async sendToolResult(
    ...
    console.log(`Tool result sent for session ${sessionId}:`, result);
  }
